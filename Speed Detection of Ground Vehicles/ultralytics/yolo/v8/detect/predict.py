...
def estimatespeed(Location1, Location2):
    ...
    print("!!!!d_pixel: ", str(d_pixel))
    #defining the pixel per meter
    #ppm = 15
...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        print("!!!!!!!!!!!!!!!!data_deque id: "+str(data_deque[id]))
        if len(data_deque[id]) >= 2:                             #if the vehicle moves
          ...
          if intersect(data_deque[id][0], data_deque[id][1], line[0], line[1]):
              ...
              if "North" in direction:
                if obj_name not in object_counter1:
                    ...
                    object_counter1[obj_name] += 1
        try:
            print("speed_line_queue[id]: ", speed_line_queue[id])
            print("len speed_line_queue[id]: ",len(speed_line_queue[id]))
            print("speed on display: ",str(sum(speed_line_queue[id])//len(speed_line_queue[id])))
            label = label + " " + str(sum(speed_line_queue[id])//len(speed_line_queue[id])) + "miles/h"  #averaging the speed through the time
        except:
            pass
